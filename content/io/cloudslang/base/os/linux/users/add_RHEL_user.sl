    ...
    - add_user:
        do:
          ssh.ssh_flow:
            ...
            - group_name_string: ${'' if group_name == '' else ' -g ' + group_name}
            - create_home_string: ${'' if create_home in [True, true, 'True', 'true'] else ' --no-create-home '}
            - home_path_string: >
                ${'/home' if (home_path == '' and create_home in [True, true, 'True', 'true']) else ' --home-dir ' +
                home_path}
            - command: >
                ${'adduser ' + user_name + create_home_string + group_name_string + home_path_string + '-p' + user_password }
        publish:
         - return_result
         - standard_out
         - standard_err
